...
import (
	...
	"github.com/mcadenas-bjss/go-do-it/app/widgets"
)
...
func (a *App) newTodoList() fyne.CanvasObject {
	...
	create := func() fyne.CanvasObject {
		return widgets.NewTodoListItem()
	}
	selected := func(id widget.ListItemID) {
		fmt.Println("Selected", id)
	}
	updateItem := func(id widget.ListItemID, obj fyne.CanvasObject) {
		obj.(*fyne.Container).Objects[0].(*widget.Check).SetChecked(a.Store.data[id].Completed)
		obj.(*fyne.Container).Objects[2].(*canvas.Text).Text = fmt.Sprintf("list item %d is %d::%s", id, a.Store.data[id+1].Id, a.Store.data[id+1].Description)
	}
